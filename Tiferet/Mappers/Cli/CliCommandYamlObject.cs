...
/// <summary>Transfer object for <see cref="CliArgumentConfiguration"/> in YAML configuration.</summary>
public class CliArgumentYamlObject : TransferObject
{
    ...
    public CliArgumentConfiguration ToRecord() => new(
        NameOrFlags,
        Description,
        Enum.TryParse<CliArgumentType>(Type, true, out var t) ? t : CliArgumentType.String,
        Required, Default, Choices?.ToList(), Nargs,
        Enum.TryParse<CliArgumentAction>(Action, true, out var a) ? a : null);

    public static CliArgumentYamlObject FromRecord(CliArgumentConfiguration arg) => new()
    {
        NameOrFlags = arg.NameOrFlags.ToList(), Description = arg.Description,
        Type = arg.Type.ToString(), Required = arg.Required, Default = arg.Default,
        Choices = arg.Choices?.ToList(), Nargs = arg.Nargs, Action = arg.Action?.ToString(),
    };
    ...
    public static CliArgumentYamlObject FromYaml(Dictionary<string, object> data)
    {
        ...
        if (data.TryGetValue("Type", out var tObj)) obj.Type = tObj?.ToString() ?? "String";
        if (data.TryGetValue("Required", out var rObj) && bool.TryParse(rObj.ToString(), out var r)) obj.Required = r;
        if (data.TryGetValue("Choices", out var cObj) && cObj is List<object> cList)
            obj.Choices = cList.Select(c => c.ToString()!).ToList();
        if (data.TryGetValue("Action", out var aObj)) obj.Action = aObj?.ToString();
