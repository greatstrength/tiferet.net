...
public class AppInterfaceContext : IDisposable
{
        ...
        Dictionary<string, object?>? data = null)
    {
        ...
        {
            logger.LogError("Error executing feature {FeatureId}: {Error}", featureId, e.Message);
        ...
        logger.LogDebug("Feature {FeatureId} executed successfully, handling response.", featureId);
        logger.LogInformation("Executed Feature - {FeatureId} ({DurationMs}ms)", featureId, durationMs);
